...
public class OrmTemplate {
    ...
    public <T> Optional<T> get(final DetachedCriteria criteria) {
        return Optional.ofNullable(tmpl.executeWithNativeSession(
                (session) -> (T) criteria.getExecutableCriteria(session).setCacheable(true).uniqueResult()));
    }
    ...
    public <T> PagingList<T> find(final DetachedCriteria criteria, final Pagination page) {
        return new PagingList<T>(tmpl.executeWithNativeSession((session) -> {
            ...
            Optional.ofNullable(page).ifPresent((pg) -> {
                if (page.getPage() > 0)
                    execCriteria.setFirstResult(page.getFirstResult());
                if (page.getSize() > 0)
                    execCriteria.setMaxResults(page.getSize());
                page.getSort().orders().forEach((order) -> execCriteria.addOrder(
     ...
     * <p>クロージャ戻り値は引数に取るOrmCriteriaのresult*の実行結果を返すようにしてください。
     */
    public <T> PagingList<T> find(Class<T> entityClass, Function<OrmCriteria<T>, DetachedCriteria> func,
            final Pagination page) {
        return find(func.apply(new OrmCriteria<>(entityClass)), page);
    }

    public <T> PagingList<T> find(Class<T> entityClass, String alias, Function<OrmCriteria<T>, DetachedCriteria> func,
            final Pagination page) {
    ...
    public <T> Optional<T> get(final String hql, final Object... args) {
        return Optional.ofNullable(tmpl.executeWithNativeSession(
                (session) -> (T) bindArgs(session.createQuery(hql).setCacheable(true), args).uniqueResult()));
    }
    ...
    public <T> List<T> find(final String hql, final Object... args) {
        return tmpl.executeWithNativeSession((session) -> bindArgs(session.createQuery(hql), args).list());
    }
    ...
    public <T> List<T> findNamed(final String queryName, final Object... args) {
        return (List<T>) tmpl
                .executeWithNativeSession((session) -> bindArgs(session.getNamedQuery(queryName), args).list());
    }
    ...
    public <T> List<T> findBySql(final String sql, final Object... args) {
        return tmpl.executeWithNativeSession((session) -> bindArgs(session.createSQLQuery(sql), args).list());
    }
    ...
    public <T> PagingList<T> findBySql(final String sql, final Pagination page, final Object... args) {
        return new PagingList<T>(
                tmpl.executeWithNativeSession((session) -> bindArgs(session.createSQLQuery(sql), page, args).list()),
                page);
    }

    /** HQLを実行します。 */
    public int execute(final String hql, final Object... args) {
        return tmpl.executeWithNativeSession((session) -> bindArgs(session.createQuery(hql), args).executeUpdate());
    }

    /** SQLを実行をします。*/
    public int executeSql(final String sql, final Object... args) {
        return tmpl.executeWithNativeSession((session) -> bindArgs(session.createSQLQuery(sql), args).executeUpdate());
    }
    ...
    protected Query bindArgs(final Query query, final Pagination page, final Object... args) {
        ...
        Optional.ofNullable(sessionHolder).filter((v) -> v.hasTimeout())
                .ifPresent((v) -> query.setTimeout(v.getTimeToLiveInSeconds()));
        return query;
    }
    ...
    protected void prepareCriteria(Criteria criteria) {
        ...
        SessionHolder sessionHolder = (SessionHolder) TransactionSynchronizationManager
                .getResource(tmpl.getSessionFactory());
        Optional.ofNullable(sessionHolder).filter((v) -> v.hasTimeout())
                .ifPresent((v) -> criteria.setTimeout(v.getTimeToLiveInSeconds()));
    }
    ...
    protected void prepareQuery(Query queryObject) {
        ...
        SessionHolder sessionHolder = (SessionHolder) TransactionSynchronizationManager
                .getResource(tmpl.getSessionFactory());
        Optional.ofNullable(sessionHolder).filter((v) -> v.hasTimeout())
                .ifPresent((v) -> queryObject.setTimeout(v.getTimeToLiveInSeconds()));
    }
